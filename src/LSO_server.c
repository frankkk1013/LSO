...
bool login(LpClientInfo clientInfo)
{
	...
	do {
		...
		/*Ciclo sul file database per verificare che lo username non esista già */
		do { /*La lettura avviene un carattere alla volta fintanto non viene trovato un newline */
			alreadyLogged = false;
			if ((bytesReaded = read(database, character, 1)) > 0)
	...
	if (tmp != NULL)
	{
		...
		while (tmp->nextClientInfo)
		{
			...
			if (tmp->status == CLTINF_LOGGED)
			{
				...
				while (traverse)
				{
					if (traverse->status == CLTINF_LOGGED)
					{
						...
						if (dist < DISTANCE && (tmp->infected == true || traverse->infected == true))
						{
							...
							if (traverse->cycles == CYCLE && traverse->infected == false)
							{
